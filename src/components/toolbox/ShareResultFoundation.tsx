...
export interface ResultShareAdapter { title: string; publicUrl: string; imageUrl?: string; }

export default function ShareResultFoundation({ monster, contentType, slug, title }: Props) {
  const publicUrl = typeof window === 'undefined' ? '' : window.location.href.split(/[?#]/)[0];
  const copy = async () => {
    try { await navigator.clipboard.writeText(publicUrl); trackShareAction({ monster, content_type: contentType, slug, action: 'copy_link' }); } catch { /* Clipboard can be unavailable; the visible URL remains shareable. */ }
  };
  const share = async () => {
    if (!navigator.share) return copy();
    try { await navigator.share({ title, url: publicUrl }); trackShareAction({ monster, content_type: contentType, slug, action: 'web_share' }); } catch { /* A dismissed system dialog is not an error state. */ }
  };
  return <div className="share-foundation" aria-label={`Share ${title}`}>
    <span>Share this page</span><button type="button" onClick={share}>Share</button><button type="button" onClick={copy}>Copy link</button>
